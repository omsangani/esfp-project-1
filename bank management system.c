...

{
    ...
    add_invalid:
    reset();
    green();printf("\n\n\n\t\tEnter 1 to go to the main menu and 0 to exit:");reset();
    scanf("%d",&main_exit);
    // system("cls");
    if (main_exit==1)
        menu();
...
yellow();
    ...
    while (fscanf(old,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d",&add.acc_no,add.name,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF)
   {
            ...
            {   test=1;
                ...
                {
                    red(); printf("\a\a\a\n\nYOU CANNOT DEPOSIT OR WITHDRAW CASH IN FIXED ACCOUNTS!!!!!"); reset();
                    fordelay(1000000000);
                    // system("cls");
                    menu();
...
void erase(void)
{
   ...
   if(test==0)
        {   red();
            ...
            erase_invalid:
            green();
              printf("\nEnter 0 to try again,1 to return to main menu and 2 to exit:");reset();
              scanf("%d",&main_exit);
...
void close(void)
{
    ...
    printf("\n\n\n\n\n\n\n\n -created by Om Rathod, Om Sangani, Yash Radadiya");
    reset();
    }
...
int main()
{
    char pass[10],password[10]="oyasho";
    int i=0;
